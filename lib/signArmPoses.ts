...
/**
 * Per-sign arm + head poses layered UNDER captured mocap finger data.
 *
 * VRM 1.0 normalized bone axis convention (verified empirically on our
 * avatar, VRM1_Constraint_Twist_Sample.vrm):
 *   - leftUpperArm.z NEGATIVE  → arm swings DOWN to the side (rest)
 *   - leftUpperArm.z POSITIVE  → arm swings UP overhead
 *   - rightUpperArm.z POSITIVE → arm swings DOWN to the side (rest)
 *   - rightUpperArm.z NEGATIVE → arm swings UP overhead
 *   - leftLowerArm.y  NEGATIVE → elbow flexes FORWARD (hand toward face)
 *   - leftLowerArm.y  POSITIVE → elbow hyperextends backward (wrong way)
 *   - rightLowerArm.y POSITIVE → elbow flexes FORWARD (hand toward face)
 *   - rightLowerArm.y NEGATIVE → elbow hyperextends backward (wrong way)
 */
...
export const ARM_POSES: Record<string, ArmPose> = {
  // HELLO — right arm raised up + slightly forward, wave in hand/fingers
  HELLO: mergeRest({
  ...
  // THANK-YOU — flat palm near chin, arm up+forward
  "THANK-YOU": mergeRest({
  ...
  // FRIEND — both hands forward, interlocking hook
  FRIEND: mergeRest({
