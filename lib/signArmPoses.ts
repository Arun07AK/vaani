 ...
 *
 * All rotations are VRM-humanoid Eulers in radians, XYZ order. Default
 * "arms at sides" = leftUpperArm.z=+1.3, rightUpperArm.z=-1.3.
 */
...
export const ARM_POSES: Record<string, ArmPose> = {
  // HELLO — right arm raised up, wave motion in hand/fingers
  HELLO: mergeRest({
  ...
  // THANK-YOU — flat palm to chin, both arms partially forward
  "THANK-YOU": mergeRest({
  ...
  // NAME — two hands form "N" tap across each other
  NAME: mergeRest({
    leftUpperArm: [-0.9, -0.3, 0.9],
    rightUpperArm: [-0.9, 0.3, -0.9],
